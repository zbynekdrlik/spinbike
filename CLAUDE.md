...
- Expired monthly pass auto-renewing on the next visit (door press / class charge) → `.claude/rules/pass-auto-renewal.md` (auto-loads on `routes/door.rs`, `jobs/charger.rs`, `db/users.rs`, `tests/door_route.rs` — the shared `auto_renew_pass` helper, staff_id-NULL distinguisher, never-had-pass boundary, no-double-debit gating, #365)
