...
async fn day_report_pagination_has_more_true_when_more_than_limit() {
    ...
    // limit=2, seeded=3 → events.len()=2, has_more=true
    let (status, body) = app
        .request(get(
...
async fn day_report_card_name_is_null_when_names_empty() {
    // Guards against: `delete !` mutation in `.filter(|s| !s.trim().is_empty())`.
    // Customer card from TestApp has no first_name/last_name — card_name must be null.
    let app = TestApp::new().await;
    let card_id = app.customer_card_id;
    sqlx::query(
        "INSERT INTO transactions (card_id, amount, action, created_at) VALUES (?1, -5.0, 'charge', datetime('now'))",
    ...
    let (status, body) = app
        ...
        .await;
    assert_eq!(status, StatusCode::OK);
    let evt = &body["events"][0];
    assert!(
        evt["card_name"].is_null(),
        "card_name must be null for card with no first_name/last_name"
    );
}
