...
async fn charge_rejects_zero_amount() {
    ...
    let user_id = app.seed_card("C1", 100.0, None, None, None, None).await;
    let spinning_id = app.spinning_service_id().await;

    let body = serde_json::json!({ "user_id": user_id, "amount": 0.0, "service_id": spinning_id });
    let (status, resp) = app
...
async fn charge_rejects_negative_amount() {
    let app = TestApp::new().await;
    let user_id = app.seed_card("C2", 100.0, None, None, None, None).await;
    let spinning_id = app.spinning_service_id().await;

    let body = serde_json::json!({ "user_id": user_id, "amount": -5.0, "service_id": spinning_id });
    let (status, resp) = app
        .request(post_json("/api/payments/charge", &app.staff_token, &body))
        .await;
    assert_eq!(status, axum::http::StatusCode::BAD_REQUEST);
...
async fn charge_reduces_credit_by_exact_amount() {
    let app = TestApp::new().await;
    let user_id = app.seed_card("C3", 100.0, None, None, None, None).await;
    let spinning_id = app.spinning_service_id().await;

    let body = serde_json::json!({ "user_id": user_id, "amount": 10.0, "service_id": spinning_id });
    let (status, resp) = app
        .request(post_json("/api/payments/charge", &app.staff_token, &body))
        .await;
    ...
    // Verify the user credit persisted correctly (kills mutants on the balance math).
    let persisted: f64 = sqlx::query_scalar("SELECT credit FROM users WHERE id = ?")
        .bind(user_id)
        .fetch_one(&app.pool)
        .await
        .unwrap();
...
async fn charge_forbidden_for_customer() {
    let app = TestApp::new().await;
    let user_id = app.seed_card("C4", 100.0, None, None, None, None).await;
...
async fn storno_rejects_zero_amount() {
    let app = TestApp::new().await;
    let user_id = app.seed_card("S1", 100.0, None, None, None, None).await;
...
async fn storno_rejects_negative_amount() {
    let app = TestApp::new().await;
    let user_id = app.seed_card("S2", 100.0, None, None, None, None).await;
    let body = serde_json::json!({ "user_id": user_id, "amount": -3.0 });
    let (status, _) = app
        .request(post_json("/api/payments/storno", &app.staff_token, &body))
        .await;
    assert_eq!(status, axum::http::StatusCode::BAD_REQUEST);
...
async fn storno_increases_credit_by_exact_amount() {
    let app = TestApp::new().await;
    let user_id = app.seed_card("S3", 50.0, None, None, None, None).await;
    let body = serde_json::json!({ "user_id": user_id, "amount": 20.0 });
    let (status, resp) = app
        .request(post_json("/api/payments/storno", &app.staff_token, &body))
        .await;
    assert_eq!(status, axum::http::StatusCode::OK);
...
async fn storno_forbidden_for_customer() {
    let app = TestApp::new().await;
    let user_id = app.seed_card("S4", 50.0, None, None, None, None).await;
    let body = serde_json::json!({ "user_id": user_id, "amount": 5.0 });
    let (status, _) = app
        .request(post_json(
            "/api/payments/storno",
...
#[tokio::test]
async fn charge_blocked_user_is_conflict() {
    let app = TestApp::new().await;
    let user_id = app.seed_card("BLK", 100.0, None, None, None, None).await;
    let spinning_id = app.spinning_service_id().await;
    // Block the user directly in the DB.
    spinbike_server::db::users::set_blocked(&app.pool, user_id, true)
        .await
        .unwrap();
...
#[tokio::test]
async fn charge_missing_user_is_404() {
    let app = TestApp::new().await;
    let spinning_id = app.spinning_service_id().await;
    let body = serde_json::json!({ "user_id": 999_999, "amount": 10.0, "service_id": spinning_id });
    let (status, _) = app
        .request(post_json("/api/payments/charge", &app.staff_token, &body))
        .await;
...
async fn payment_routes_are_registered() {
    let app = TestApp::new().await;
    // Seed a real user so handler's "User not found" 404 can't mask a
    ...
    let storno_body = serde_json::json!({ "user_id": user_id, "amount": 5.0 });
    let (status, _) = app
        .request(post_json(
            "/api/payments/storno",
            &app.staff_token,
        ...
        .await;
    assert_eq!(status, axum::http::StatusCode::OK);
    // Touch /api/users so the `get` helper is exercised in this binary too.
    let _ = app.request(get("/api/users", &app.staff_token)).await;
}
