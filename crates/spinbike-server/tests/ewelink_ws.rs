...
/// Verifies that after a successful press the `last_ack_ms` atomic is
/// populated. Indirect coverage for the `Some(0) =>` arm in `handle_text_frame`
/// (an `Ok(())` produced WITHOUT setting last_ack_ms would leave the sentinel
/// in place — this test fails in that case).
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn mock_ws_press_updates_last_ack_ms() {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let ws_url = format!("ws://{addr}/dispatch/app");
    ...
    let server = tokio::spawn(async move {
        ...
        ws.send(Message::Text(r#"{"error":0,"apikey":"k"}"#.into()))
            .await
            .unwrap();
        // update press → ok with same sequence
        let msg = ws.next().await.unwrap().unwrap();
        let text = msg.to_text().unwrap().to_string();
        let seq = text
            .split("\"sequence\":\"")
    ...
    assert_eq!(
        before,
        i64::MIN,
        "before press, last_ack_ms must hold the sentinel"
    );
    ...
    assert_ne!(
        after,
        i64::MIN,
        "after a successful ack last_ack_ms must be populated"
    );
    assert!(
        after > 0,
        "last_ack_ms should be a positive timestamp, got {after}"
    );
