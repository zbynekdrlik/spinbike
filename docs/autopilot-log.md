...
## 2026-08-08 — #306 + #305: wrong low-credit push during an active pass + a lying per-device switch (bundled batch, dev.140)

- **#306 (priority, bug, self-reported by the owner's own account):** `jobs::notifications::tick_as_of`'s `low_credit` loop gated only on `credit`/`last_topup`, never on pass status — while a pass is active, credit is 0 by design (visits booked `action='visit'` amount 0), so a pass holder whose last top-up cleared the 20 EUR gate got "Dochadza ti kredit" for no reason. Proven live on prod BEFORE any code (STEP 0 comment on #306): the ticket's own SQL query still returned 26 matching customers, user 535 (the owner) still had a stale `push_notify_log` row from that morning's tick.
- **Fix:** low_credit now also reads `db::users::get_user_pass_valid_until` (same canonical `user_active_pass` view V18 `pass_expiring` already reads) and adds `&& !has_active_pass` (`pass_valid_until >= today`, inclusive of the expiry day itself). Same per-row DB-error resilience pattern as the existing `last_topup_amount` check two lines above (log + `continue`, never abort the batch). `RED` `aa427c2` (3 new tests: active-10-days-out sends 0, expired-yesterday sends 1, exactly-today boundary sends 1-but-it's-pass_expiring-not-low_credit) -> `GREEN` `10efddb`.
- **A pre-existing test had literally encoded the bug as expected behavior** (`low_credit_and_pass_expiring_are_independent_for_the_same_user` asserted `sent == 2` for a low-credit-AND-active-pass customer) — renamed to `..._can_never_co_fire_for_the_same_user` and rewritten (own commit `2537e8f`) to lock the real invariant: `pass_expiring`'s window is a strict subset of the new active-pass condition, so the two reasons are now structurally mutually exclusive per user (documented in `.claude/rules/push-notifications.md`).
- **#305 (bug, flagged `needs-user-decision` at filing but resolved as a pure engineering call, not a real product fork):** `GET /api/push/config`'s `subscribed` flag is a per-ACCOUNT aggregate (`COUNT(*) FROM push_subscriptions WHERE user_id=?` > 0) — a second device that never itself subscribed inherited "On" from a different device's row. Fix: mount effect asks the browser's OWN `PushManager.getSubscription()` (new `local_subscription_fields()`/`get_local_subscription()`) instead of trusting the server flag; reconciles a live local subscription to the server with an idempotent upsert. `PushConfigResp` no longer deserializes `subscribed` at all. `RED` `5bff3fb` (new e2e: server shows `subscribed:true` from a DIFFERENT endpoint, this device's own `getSubscription()` genuinely unstubbed/empty, must still POST its OWN subscribe) -> `GREEN` `72a59c8`.
- **Three review-fix commits after two deep-review passes (a fast `/review`, a `requesting-code-review` deep pass, and a separate 8-finder/4-verifier multi-agent pass) found real reliability gaps, all fixed in-branch:** `370e21e` (stale doc line, console.warn on a silently-swallowed reconcile POST) -> `6cda832` (two `unsubscribe_flow()` fallback branches fell through to `Ok(endpoint)` on a malformed/non-Promise `unsubscribe()`, contradicting the function's own doc'd invariant — now `Err(())`; `get_local_subscription()`'s `sw.ready()` await ran UNCONDITIONALLY on every mount with no bound — added `get_local_subscription_bounded()`, a `futures::future::select` race against `gloo_timers::TimeoutFuture`, 5s) -> `022bcc7` (extracted `ready_push_manager()`, `subscribe_flow` and `get_local_subscription` had duplicated the SW-ready acquisition sequence verbatim). Findings deliberately left as-is, with reasoning recorded on #305's own review comment: a pre-existing (since #303) inaccurate self-heal comment unrelated to this diff's actual behavior change; `PUSH_USER_DISABLED_KEY` being client-only localStorage (an explicit #303 owner decision); a narrow double-fetch on the self-heal edge path; N+1 vs `pass_expiring`'s batched query (accepted at this project's stated scale).
- **Commits (dev):** `9589cfe` (bump 0.15.0-dev.140) -> `aa427c2` [red #306] -> `10efddb` [green #306] -> `5bff3fb` [red #305] -> `72a59c8` [green #305] -> `2537e8f` (test-fix: the bug-encoding pre-existing test) -> `370e21e`/`6cda832`/`022bcc7` (review-fix commits). PR [#308](https://github.com/zbynekdrlik/spinbike/pull/308) (Closes #306, Closes #305), merged `d337d08`. Main CI green incl. E2E, all 8 mutation shards, Deploy (prod), Smoke (prod).
- **Verified LIVE on prod DB + `https://spinbike.sk` (v0.15.0-dev.140):** `journalctl` showed `push: daily tick complete sent=0` right after the deploy restart's startup tick; user 535's stale `low_credit` `push_notify_log` row (present before the fix) was gone after, and the 26-customer wrong-state query's own matches now have ZERO `push_notify_log` rows for `low_credit` (the query itself still returns 26 — it describes DB STATE the fix doesn't change, only notification BEHAVIOR does). Playwright against a synthetic customer on `/my/balance`: DOM version matched, push card rendered without crashing/hanging (state `blocked` — this MCP profile's own prior-session permission state, not a bug), 0 console errors/warnings. Synthetic user + scratch JWT-secret/token files deleted after.
- **Playbook-only follow-up (`e473846` bump to dev.141), PR [#309](https://github.com/zbynekdrlik/spinbike/pull/309) (no issue, pure playbook maintenance), merged `e72ad0b`:** captured the #306 mutual-exclusion invariant in `.claude/rules/push-notifications.md`, and two #305 gotchas in `.claude/skills/frontend-pwa/SKILL.md` — per-account server flags can never answer per-device questions, and the `futures::future::select`+`gloo_timers::TimeoutFuture` pattern for bounding a possibly-hanging browser Promise (both crates were already direct `spinbike-ui` deps, just never combined into a race before). Deployed + verified (v0.15.0-dev.141, `Deploy (prod)`+`Smoke (prod)` both success).

## 2026-08-10 — #311: e-mail fallback when a customer has no push subscription (solo, dev.143/dev.144)

- **Owner decision recorded on the ticket (variant (a), 2026-08-08/10):** e-mail is a FALLBACK ONLY, never a duplicate channel. Customer has ≥1 stored push subscription -> push only, never e-mail (even if that subscription is failing THIS tick — channel choice is made on subscription EXISTENCE, not per-tick delivery success; the existing `MAX_CONSECUTIVE_FAILURES` prune already handles a genuinely dead subscription, after which the customer naturally falls onto e-mail on a later tick). No push subscription + `users.email` present -> exactly one e-mail. No push subscription + `users.email IS NULL` (legacy card-migrated accounts) -> skip silently, no error, no ledger write.
- **Design:** `evaluate_reason`'s existing `if subs.is_empty() { return Ok(false); }` early-return became the e-mail-fallback arm; the two per-user SELECT queries (`credit_rows`, `pass_rows`) grew one more column (`email`) instead of a new N+1 query. Ledger (`push_notify_log`, unchanged schema — already channel-agnostic) stamped only on `Ok(())` from `MailHandle::send`, mirroring the push path's `SendOutcome::Sent`-only stamping. `MailHandle` gained two env-free test constructors (`capture()`/`disabled()`), mirroring `PushHandle::from_base64_private_key`'s rationale — `jobs::notifications` tests must not race `mail::tests`'s own `SMTP_*` env mutations.
- **RED->GREEN commit order:** `a2c4390` [red] (4 new tests + the mail plumbing/test-constructor scaffolding, every pre-existing test updated to pass `MailHandle::disabled()` so behavior is provably unchanged) -> `80072d4` [green] (the actual fallback branch).
- **CI-caught compile gap (`43a90e8`):** the RED commit's mechanical find-and-replace over ~24 test call sites matched `tick_as_of(&pool, &push, test_today())`/`tick_as_of(&pool, &push, today)` but missed 6 more call sites inside `low_credit_full_anti_spam_sequence_cooldown_episode_cap_then_rearm`, which uses its own locally-scoped `day0`/`day8`/`day16` variables — caught only by CI's `Lint` (clippy) job, since Tier-0 forbids local `cargo build`/`clippy`/`test` on this project. Playbook captures the "grep by stable prefix, not trailing-arg literals" gotcha (see below).
- **Two independent deep-review passes** (`requesting-code-review` general-purpose subagent + the `code-review` skill, both dispatched in parallel against the full diff) found real issues, all fixed in-branch (`a150398`, `4ee25bc`): trim the fallback address before sending (defensive — every current write path already trims `users.email`, no live bug); log the destination on a failed send; a pre-existing `captured.lock().expect(...)` in `mail/mod.rs` violated this project's own CLAUDE.md #172 mutex-recovery mandate (fixed to `.unwrap_or_else(|poisoned| poisoned.into_inner())`, matching `door.rs`/`auth.rs`); `capture()`/`disabled()` duplicated `spawn()`'s struct-literal shapes (factored into a shared private `with_transport`); added a test proving a present-but-failing-this-tick subscription never falls back to e-mail (the existing guard test's mock always returned 201, so it couldn't catch that specific regression). Three suggestions (mirror the email arm's control flow to the push loop's shape; a generalized multi-channel abstraction for a hypothetical future SMS channel; HTML escaping) evaluated and declined with reasoning, posted to #311.
- **Commits (dev):** `4641502` (bump 0.15.0-dev.143) -> `a2c4390` [red] -> `80072d4` [green] -> `ef9b675` (docs) -> `a150398`/`43a90e8`/`4ee25bc` (review-fix + CI-caught-gap commits). PR [#312](https://github.com/zbynekdrlik/spinbike/pull/312) (Closes #311), merged `68e5a0a`. Main CI: one unrelated E2E flake (`visit-button-feedback.spec.ts`, a 1000ms button-disabled timing assertion — same code had passed 229/229 on the dev-branch run moments earlier) fixed by a single job rerun, no code change; second attempt green incl. all 8 mutation shards, Deploy (prod), Smoke (prod).
- **Verified LIVE on prod (v0.15.0-dev.143) — a genuinely strong confirmation, for free, from the daily job's own post-deploy startup tick:** `journalctl` showed 15 real customers notified via the new e-mail fallback (`mail: sent to=... subject=Dochadza ti kredit`/`subject=Permanentka ti konci o N dni`) AND the exact failure-retry path firing live — one customer's SMTP send timed out, followed immediately by `push: email fallback send failed, ledger not stamped (retried next tick)`, no panic, no crash. `push: daily tick complete sent=17`. Playwright at 390x844 (stale SW/cache cleared first): DOM version matched, 0 console errors. No synthetic DB rows involved — this ticket's verification is read-only (DOM check + reading the real daily job's own journal trace), unlike `/my/*` customer-facing tickets that need a synthetic throwaway account.
- **Playbook-only follow-up (`05cccf8` bump to dev.144), PR [#313](https://github.com/zbynekdrlik/spinbike/pull/313) (no issue, pure playbook maintenance), merged `3b5f8a7`:** captured the mechanical-replace gotcha in `.claude/rules/push-notifications.md` — when widening a function's signature across many test call sites, grep by the call's STABLE PREFIX (e.g. `tick_as_of(&pool, &push,`) and count matches against the known test-function total, never by trailing-argument literals that vary per test. Deployed + verified (v0.15.0-dev.144, `Deploy (prod)`+`Smoke (prod)` both success, `/api/version` matches).

## 2026-08-10 — #319: single-row customer header with burger menu (solo, dev.148)

- **Owner report (photo of live prod at 390x844):** the 2-row + a separate "Ahoj, {name}" row customer header ate ~21% of the first screen (145px + ~4%) vs Credit+Monthly-pass cards' own ~20%.
- **Root cause + design (posted BEFORE the first code commit):** `Navbar` rendered every customer control (logo, 3 links, name, Logout, lang toggle) as always-visible inline items with no collapsing mechanism; a mobile media query forced `.navbar-links` onto its own full-width row regardless of content. Fix: collapse the 3 links + Logout + lang toggle into a burger-triggered `Sheet` (reusing the SAME component `AdaptiveNav` already uses for staff's "More" sheet — zero changes to that path). New `short_display_name()` in `i18n.rs` ("Firstname Lastname" -> "Firstname L.", unicode-safe, panic-free on empty/single-word names). Real focus management (poll for the Sheet's DOM mount rather than a single fixed-delay wait — a genuine CI-caught race, fixed same-branch in `2861bfe`). Removed the standalone "Ahoj, {name}" `<h1>` on `/my/balance`.
- **Explicitly out of scope, verified UNTOUCHED in the final diff:** door-unlock button, "Add to home screen" button, card order, `AdaptiveNav`/staff header (zero diff on `adaptive_nav.rs`).
- **Commits (dev):** `63bde50` (bump 0.15.0-dev.148) -> `fb7dc08` [red] (`nav-burger-customer.spec.ts`, single-row header + burger with all 5 items + name shortening) -> `cd15c37` [green] -> `2861bfe` (CI-caught focus-mount race, bounded poll not a fixed delay) -> `65e957b` (review-fix: explicit menu-close-on-click defense-in-depth; a pre-existing `Sheet` focus-trap gap correctly filed as #320 rather than scope-creeped in). PR [#321](https://github.com/zbynekdrlik/spinbike/pull/321) (Closes #319), merged `a5b79bd9`. Main CI green incl. E2E, all 8 mutation shards, Deploy (prod), Smoke (prod).
- **Two review passes before merge:** `requesting-code-review` (general-purpose deep reviewer, full diff) — 0 Critical, 0 Important, 2 Minor non-blocking; verdict "Ready to merge: Yes". The `/code-review` skill's `--target dev` argument did NOT scope to the PR diff as intended — it audited the WHOLE `dev` tree (349 files) instead and surfaced 10 real, pre-existing, UNRELATED bugs (spot-checked 2, both confirmed real). None touched a file in #319's diff, so none were fixed here (would have been scope creep); all 10 filed as tracked issues (#322-#331, each `Scope-gate: needs-user-decision`/`cross-cutting`) per `no-dropped-work.md` instead of being dropped.
- **Verified LIVE on prod (v0.15.0-dev.148) with a REPRESENTATIVE synthetic customer** (two-word name so the shortened form is actually visible, `allow_self_entry=1` so the door button renders ACTIVE — the earlier misleading screenshot used a permission-less test account): header `145px -> 69px` (17% -> 8.2% of a 844px screen), Credit+Monthly-pass cards combined `21.5%`, burger opens all 5 items (Moje rezervacie/Zostatok/Nastavenia/EN/Odhlasit), no "Ahoj" row, door button rendered active+blue ("🔓Otvorit dvere - drz 1s", not disabled), 0 console errors/warnings. Screenshot: http://100.104.8.125:8788/12eEfrwJtpOM1IO2qv48sw/319-balance-header.png. Synthetic user + all related rows deleted after (0 remaining, verified by query).
- **Playbook-only follow-up (`df1260a` on dev.148, no version bump — docs only):** captured in `.claude/skills/prod-verification/SKILL.md` — a long-lived Playwright MCP `mcp-chrome-*` profile can carry a stale session from an EARLIER, unrelated verification run; plain `localStorage.clear()` + SW-unregister is not reliable enough, needs a real CDP `Storage.clearDataForOrigin` wipe (+ closing any duplicate tabs on the same origin) before injecting a fresh synthetic session.

## 2026-08-11 — round: #320 + #325 + #326 + #327 + #330 (v0.15.0-dev.150)

Three parallel worktree workers, one serial integration, one PR.

- **#320** Sheet had no Tab-focus trap. Added a real Tab/Shift+Tab boundary wrap in the shared `Sheet` keydown handler (`spinbike-ui/src/components/sheet.rs`), so all 8 call sites get it. Disabled / `tabindex=-1` elements excluded from the boundary; the focusable selector is now shared with `nav.rs` instead of duplicated. Follow-up #334 filed for the narrow all-descendants-disabled transient window (needs live E2E across all 8 call sites).
- **#325** `topup_user` rounded the credit delta but wrote the raw amount to the ledger. Round once, reuse for both writes.
- **#326** Door no-pass charge was the only money-mutating write without rounding. Round once; `ROUND(...,2)` in the SQL as defense-in-depth.
- **#327** `materialiser::sweep()` was the last production site computing "today" from the OS zone. Swapped to `crate::util::today_bratislava()`.
- **#330** `patch_created_at` did the same, plus hand-rolled the DST local->UTC conversion. Both now route through the shared helper; the DST logic was generalized into `crate::util::bratislava_local_to_utc()` (one tested implementation instead of two copies).
